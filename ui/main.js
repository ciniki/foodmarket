...
/*        'supplier_products':{'label':'Products', 'type':'simplegrid', 'num_cols':2, 'sortable':'yes',
        ...
        'customers':{'label':'Customers', 'type':'simplegrid', 'num_cols':1, 'aside':'yes',
            'visible':function() { 
                var t=M.ciniki_foodmarket_main.menu.sections._tabs.selected; 
                return (t=='favourites' || t == 'repeats') ? 'yes':'no'; 
                },
            'noData':'No customers.',
            },
        /* Checkout */
        ...
        'checkout_orderpayments':{'label':'Payments', 'type':'simplegrid', 'num_cols':2,
            'visible':function() { return ( M.ciniki_foodmarket_main.menu.sections._tabs.selected == 'checkout' 
                && M.ciniki_foodmarket_main.menu.sections._checkouttabs.selected == 'order' 
                && M.ciniki_foodmarket_main.menu.order_id > 0 
                && M.ciniki_foodmarket_main.menu.data.order.payment_status > 0 ) ? 'yes':'no'; },
            'noData':'No payments',
            },
        'checkout_recentledger':{'label':'Account', 'type':'simplegrid', 'num_cols':4,
            'visible':function() { return ( M.ciniki_foodmarket_main.menu.sections._tabs.selected == 'checkout' 
        ...
        'checkout_orderbuttons':{'label':'', 
            ...
            'buttons':{
                ...
                'addpayment':{'label':'Add Payment', 
                    'visible':function() {return (M.ciniki_foodmarket_main.menu.data.order.payment_status > 0 ?'yes':'no');},
                    'fn':'M.ciniki_foodmarket_main.ledgerentry.open(\'M.ciniki_foodmarket_main.menu.addPayment();\',0,60,M.ciniki_foodmarket_main.menu.customer_id);'},
        ...
        'procurement_supplier_inputs':{'label':'Items', 'type':'simplegrid', 'num_cols':4,
            ...
            'noData':'Nothing ordered',
            'headerValues':['SKU', 'Product', 'Required', 'Order'],
            },
    ...
    this.menu.cellValue = function(s, i, j, d) {
        ...
        if( s == 'procurement_supplier_inputs' ) {
            switch(j) {
                case 0: return d.sku;
                case 1: return d.name;
                case 2: return d.required_quantity_text;
                case 3: return d.order_quantity_text;
            }
        }
    ...
    this.menu.rowFn = function(s, i, d) {
        ...
        if( s == 'customer_repeats' ) {
            return 'M.ciniki_foodmarket_main.repeatitem.open(\'M.ciniki_foodmarket_main.menu.open();\',\'' + d.id + '\');'
        }
        /* Favourites */
        if( s == 'customers' ) {
    ...
    this.menu.processCheckout = function(rsp) {
        ...
        if( rsp.order != null ) {
            ...
            p.data.checkout_orderpayments = rsp.order.payments;
        } else {
            p.data.checkout_orderitems = {};
            p.data.checkout_tallies = {};
            p.data.checkout_orderpayments = {};
        }
...
//            'taxtype_id':{'label':'Tax Type', 'type':'text'},
    ...
    //
    this.repeatitem = new M.panel('Standing Order Item', 'ciniki_foodmarket_main', 'repeatitem', 'mc', 'medium', 'sectioned', 'ciniki.foodmarket.main.repeatitem');
    ...
    this.repeatitem.updateFromSearch = function(s,f,o,oid,d) {
        ...
        this.updateForm();
        this.setFieldValue('description', unescape(d));
        this.setFieldValue('quantity', '');
        this.setFieldValue('next_order_date', '');
    } 
    ...
    this.repeatitem.open = function(cb, iid, cid, list) {
        ...
        M.api.getJSONCb('ciniki.poma.repeatItemGet', {'business_id':M.curBusinessID, 'item_id':this.item_id}, function(rsp) {
            if( rsp.stat != 'ok' ) {
                M.api.err(rsp);
                return false;
            }
    ...
    this.repeatitem.save = function(cb) {
        ...
        if( this.item_id > 0 ) {
            var c = this.serializeForm('no');
            if( c != '' ) {
                M.api.postJSONCb('ciniki.poma.repeatItemUpdate', 
                    {'business_id':M.curBusinessID, 'item_id':this.item_id, 'object':this.object, 'object_id':this.object_id}, 
                    c, function(rsp) {
                        if( rsp.stat != 'ok' ) {
                            M.api.err(rsp);
            ...
            var c = this.serializeForm('yes');
            M.api.postJSONCb('ciniki.poma.repeatItemAdd', 
                {'business_id':M.curBusinessID, 'order_id':this.order_id, 'object':this.object, 'object_id':this.object_id}, c, function(rsp) {
                    if( rsp.stat != 'ok' ) {
                        M.api.err(rsp);
                        return false;
    ...
    this.repeatitem.remove = function() {
        if( confirm('Are you sure you want to remove repeatitem?') ) {
            M.api.getJSONCb('ciniki.poma.repeatItemDelete', {'business_id':M.curBusinessID, 'item_id':this.item_id}, function(rsp) {
                if( rsp.stat != 'ok' ) {
                    M.api.err(rsp);
                    return false;
                }
    ...
    this.ledgerentry.open = function(cb, eid, t, cid) {
