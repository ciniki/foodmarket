...
function ciniki_foodmarket_dateItemDelete(&$ciniki) {
    ...
    $rc = ciniki_foodmarket_checkAccess($ciniki, $args['business_id'], 'ciniki.foodmarket.dateItemDelete');
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    ...
    // Remove the dateitem
    //
    $rc = ciniki_core_objectDelete($ciniki, $args['business_id'], 'ciniki.foodmarket.dateitem', $dateitem['id'], $dateitem['uuid'], 0x04);
    if( $rc['stat'] != 'ok' ) {
