...
// <rsp stat='ok' />
//
function ciniki_foodmarket_categoryUpdate(&$ciniki) {
    //
    ...
    $rc = ciniki_foodmarket_checkAccess($ciniki, $args['business_id'], 'ciniki.foodmarket.categoryUpdate');
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    ...
    if( isset($args['name']) ) {
        ...
        if( $rc['num_rows'] > 0 ) {
            return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.foodmarket.12', 'msg'=>'You already have an category with this name, please choose another.'));
        }
    }
    ...
    $rc = ciniki_core_objectUpdate($ciniki, $args['business_id'], 'ciniki.foodmarket.category', $args['category_id'], $args, 0x04);
    if( $rc['stat'] != 'ok' ) {
